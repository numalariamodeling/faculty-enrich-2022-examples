...
```{r}
####################################################
### View sample of available data for one survey ###
####################################################

#countryIds='BF' # selected country

#indicatorIds='' # leave blank for all indicators, or list subset c('i1', 'i2', ...)

surveyIds = 'BF2010DHS' # survey(s) to search 
# Survey ID codes can be found here: https://api.dhsprogram.com/rest/dhs/surveys?f=html

### Build Query ###
base_url <- "http://api.dhsprogram.com/rest/dhs/data?f=json"

url <- paste(base_url,
             "&surveyIds=",paste(surveyIds,collapse=','),
             end_url,
             sep = '')
url <- (url)
...
dhs_data %>%
  select(SurveyId, IndicatorId, Indicator, IsPreferred, Value, CILow, CIHigh, CharacteristicLabel, ByVariableLabel) %>%
  datatable()
```
